 ...
 * @𝗽𝗿𝗼𝗷𝗲𝗰𝘁_𝗻𝗮𝗺𝗲 : CYTRA3 𝗠𝗗, 𝗮 𝘀𝗶𝗺𝗽𝗹𝗲 𝗮𝗻𝗱 𝗲𝗮𝘀𝘆 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 𝘂𝘀𝗲𝗿 𝗯𝗼𝘁 
 * @𝗼𝘄𝗻𝗲𝗿: Cytra_k9
...
france (
    {
        ...
        if (msgRepondu) {
            if(msgRepondu.audioMessage) {
                ...
                repondre('the command only works with audio messages and learn to use your brain')
            }

        } else {
            repondre('Please mention an audio and learn to use your brain')
        }
    }
);
    ...
    async (dest, zk, commandeOptions) => {
      ...
      if (msgRepondu) {
        if (msgRepondu.audioMessage) {
          ...
          repondre("The command only works with audio messages only you mf");
        }
      } else {
        repondre("mention an audio you fool");
      }
    }
  );
...
france(
    ...
    async (dest, zk, commandeOptions) => {
      ...
      if (msgRepondu) {
        ...
        repondre("mention an audio you mf");
      }
    }
  );
